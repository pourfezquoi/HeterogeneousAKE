...
int main(void)
{
  ...
  for (int j=0;j<ALGNUMS_KEM_NIST;j++) {
    ...
    uint8_t sendA[kem_cpa->length_public_key + kem_cca->length_ciphertext + sig->length_signature];
    size_t sendAlen = 0;
    ...
    TIME_OPERATION_ITERATIONS(
      ake_send_A(kem_cpa, kem_cca, sig, lpka, lska, lpkb, epka, sendA, &sendAlen, st),
      "init",
      NTESTS
    )
      
    TIME_OPERATION_ITERATIONS(
      ake_send_B(kem_cca, kem_cpa, sig, lpka, sendA, sendAlen, lpkb, lskb, kb, sendB, &sendBlen),
      "algB",
      NTESTS
    )
    ...
    TIME_OPERATION_ITERATIONS(
      ake_receive_A(kem_cca, kem_cpa, sig, lpka, lska, lpkb, st, sendB, ka),
      "algA",
      NTESTS
    )
    PRINT_TIMER_FOOTER
...
/*
    for (int k=0; k<ALGNUMS_SIG_NIST; k++) {
      ...
      uint8_t sendA[kem_cpa->length_public_key + kem_cca->length_ciphertext + sig->length_signature];
      size_t sendAlen = 0;
      uint8_t st[kem_cpa->length_secret_key + SEED_BYTES];
      uint8_t epka[kem_cpa->length_public_key];
      uint8_t ka[SESSION_KEY_LEN], kb[SESSION_KEY_LEN];// *send = NULL;
      uint8_t sendB[kem_cpa->length_ciphertext];
      size_t sendBlen = 0;
