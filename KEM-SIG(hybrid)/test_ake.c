...
int main(){
    ...
    for (int i=0; i<ALGNUMS_KEM_NIST; i++) { 
        ...
        for (int j=0; j<ALGNUMS_SIG_NIST; j++) {
            ...
            printf("\n================ KEM: %s NIST_LEVEL: %d SIG: Ed25519\n", kem_cca->method_name, kem_cca->claimed_nist_level);
            ...
            uint8_t send[kem_cca->length_ciphertext+kem_cpa->length_ciphertext+2*SEED_BYTES];
            size_t sendlen = 0;
            ake_send_B(kem_cca, kem_cpa, lpka, epka, lpkb, lskb, kb, send, &sendlen);
            ake_receive_A(kem_cca, kem_cpa, lpka, lska, epka, eska, send, sendlen, lpkb, ka);
