...
int ake_send_A(uint8_t *sendA, uint8_t *epka, EVP_PKEY *eska, const uint8_t *lska);
...
int ake_receive_A(const uint8_t *lpka, const uint8_t *lska, 
    const uint8_t *epka, EVP_PKEY *eska, const uint8_t *recv, const size_t recvlen, const uint8_t *lpkb, uint8_t *k);